...
async function fetchWeatherData(location) {
  try {
    let response = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}}?unitGroup=us&key=2YUX44XCGJ9DV4KRCSQH7SPUR&contentType=json`
    );
...
function toCelsius(Farenheit) {
  return Math.round((5 / 9) * (Farenheit - 32.0));
}
...
document.getElementById("unit").addEventListener("click", () => {
  document.getElementById("temperature").textContent =
    conversionCounter % 2 === 0
      ? `${toCelsius(data.currentConditions.temp)} °C`
